...
class NoteTestCase(TestCase):
    ...
    def test_create_note(self):
        ...
        self.assertIsNotNone(note)  # Check that the note exists
        self.assertEqual(note.title, 'Test Note')
        self.assertEqual(note.content, 'Note content')
        self.assertEqual(note.category, self.category)
        self.assertEqual(str(note.reminder_date), '2023-07-05')
        self.assertEqual(str(note.reminder_time), '10:00:00')
    ...
    def test_edit_note(self):
        ...
        self.assertEqual(note.reminder_date.strftime('%Y-%m-%d'), '2023-07-05')
        self.assertEqual(note.reminder_time.strftime('%H:%M:%S'), '10:00:00')
        self.assertEqual(response.status_code, 302)
